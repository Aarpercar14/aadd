...
public abstract class RepositorioJPA<T extends Identificable> implements RepositorioString<T> {
	...
	public List<T> getAll() throws RepositorioException{
		...
		try {
			...
			//final String queryString = " SELECT model.id from " + getNombre() + " model "; con esto devuelve la id solo
			//final String queryString = " SELECT model.id, model.nombre from " + getNombre() + " model "; esto devolveria una lista

			TypedQuery query = em.createQuery(queryString,getClase());

			query.setHint(QueryHints.REFRESH, HintValues.TRUE); //para que no acceda a cache
